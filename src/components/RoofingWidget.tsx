...
const MapboxContainer = ({ center, zoom, addressConfirmed }: { center: { lat: number; lng: number }; zoom: number; addressConfirmed: boolean }) => {
  ...
  }, [center.lat, center.lng, zoom, hasValidToken])
    ...
    return (
      <div className="w-full h-full rounded-lg overflow-hidden bg-slate-50 border border-slate-100 flex flex-col items-center justify-center p-6 relative">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.25]">
          ...
          <div className="w-full h-px bg-gradient-to-r from-transparent via-red-300 to-transparent absolute transform rotate-45" />
          <div className="w-full h-px bg-gradient-to-r from-transparent via-red-300 to-transparent absolute transform -rotate-45" />
        </div>

        <div className="z-10 text-center space-y-4 max-w-sm">
          <div className="w-16 h-16 bg-red-50 border border-red-100 rounded-2xl flex items-center justify-center mx-auto shadow-sm shadow-red-100/30">
            <MapPin className="w-8 h-8 text-red-700 animate-bounce" />
          </div>
          <div className="space-y-1">
            <h3 className="text-slate-900 font-bold text-[16px] tracking-tight uppercase">Satellite Scan Active</h3>
            <p className="text-[12px] text-slate-500 font-semibold px-4 leading-normal">
              Scanning property coordinates:<br />
              <span className="text-red-700 font-mono text-[11px] mt-0.5 block">{center.lat.toFixed(6)}, {center.lng.toFixed(6)}</span>
            </p>
          </div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-red-50 border border-red-100 rounded-full text-[10px] text-red-700 font-bold uppercase tracking-widest">
            <div className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse" />
            Bypassing Map View (Local Dev)
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="w-full h-full relative">
      ...
      <AnimatePresence>
        {addressConfirmed && !highlightActive && (
          <motion.div 
      ...
      <div className="flex-1 px-5 lg:px-8 pb-6 lg:pb-8 flex flex-col relative pointer-events-auto">
        ...
        {currentStepId === 'NOTES' && (
          <div key="st_notes" className="space-y-6 animate-in fade-in duration-300">
            <div className="space-y-1">
              <h2 className="text-3xl font-semibold tracking-tight text-[#0F172A]">Additional Details</h2>
            </div>
            <textarea
        ...
        {currentStepId === 'LEAD_CAPTURE' && (
          <div key="st_lead" className="space-y-6 animate-in fade-in duration-300">
            <div className="space-y-2">
              <h2 className="text-3xl font-semibold tracking-tight text-[#0F172A]">Where should we send your estimate?</h2>
              <p className="text-slate-600 font-semibold italic text-[16px]">You're just one step away from your instant quote.</p>
            </div>
